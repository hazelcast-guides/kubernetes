...
Now edit `hazelcast.yaml`, and a PersistentVolumeClaim definition to match the above PersistentVolume. Finally, set the `CLASSPATH` env variable to a valid path and add a corresponding `volumeMount` in the `StatefulSet`.


== Summary

This guide shows the basic setup for Hazelcast cluster for Kubernetes. For more advanced configurations check other guides.