...
* Scale up/down Hazelcast cluster
...
At this point you have a running Hazelcast cluster with 3 members which is exposed with a service called `hz-hazelcast`.
...
Then, you can configure Hazelcast client in your application code to use your Hazelcast server DNS `hz-hazelcast`.
...
// Configure also the connection retry timout
...
With the configuration above, if you deploy your application to Kubernetes cluster, it should automatically connect into Hazelcast cluster.
...
[NOTE]
====
If you use a remote Kubernetes cluster and you want to build your own Docker image then make sure that you also push the Docker image into the Docker registry.
====
